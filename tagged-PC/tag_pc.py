...
#        print(f"seperating set of {i} and {j} is: {separating_sets[i][j]}")
    ...
    logging.debug("seperating sets Formated \n")
    for (i, j) in list(combinations(range(node_ids), 2)):
        logging.debug(f"seperating set of {i} and {j} is: {separating_sets[i][j]}")
    ...
    for (i, j) in combos:
        ...
        for k in common_k:
            ...
            if k not in sep_sets[i][j] and k in dag.successors(i) and k in dag.successors(j):
                ...
                logging.debug("V-Structure_prio_weight: ", prio_weight)
...
def typed_meek_majority_tag_with_consistency(cpdag: np.ndarray, tags: list, priomat : np.matrix, majority_rule_typed : bool = True, node_names=[], iter_max: int = 100) -> Tuple[np.ndarray, np.ndarray]: 
    ...
    # priomat checks are pretty redundant here, since a,b is undirected and should therefore have prioweight 0 but still implemented for *historic reasons*
    n_nodes = cpdag.shape[0]
    ...
    while i < iter_max:
        ...
        logging.debug(f"majority mat on t-propagation step: {i}: \n", majoritymat)
        logging.debug(f"priomat before type consistency: \n", priomat)
        logging.debug(f"adjacency mat before type consistency: \n", G)
